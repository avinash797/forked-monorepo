...
export function VariationManager({
  ...
  const [deleteTarget, setDeleteTarget] = useState<DishTypeVariationRow | null>(null);
  ...
  function updateEditing(id: string, field: keyof EditingVariation, value: string | boolean) {
    setEditing((prev) => {
  ...
  async function saveNew() {
    ...
    if (err) {
      ...
      setAddingNew(false);
      setNewRow({ id: null, name: "", slug: "", emoji: "", is_active: true, autoSlug: true });
      startTransition(() => router.refresh());
    }
  }
        ...
        <table className="w-full text-sm">
          <thead className="bg-[#1a0f08]">
            <tr>
              ...
              <th className="px-3 py-2 text-left text-xs font-semibold text-[#9BA1A6] uppercase">Emoji</th>
              <th className="px-3 py-2 text-left text-xs font-semibold text-[#9BA1A6] uppercase">Status</th>
          ...
          <tbody className="divide-y divide-[#4a3728]">
            {variations.length === 0 && !addingNew && (
              <tr>
                <td colSpan={5} className="px-3 py-4 text-center text-[#9BA1A6] text-xs bg-[#342219]">
                  No variations yet.
                </td>
              </tr>
            )}
            ...
            {variations.map((v) => {
              ...
              return (
                <tr key={v.id} className="bg-[#342219] hover:bg-[#3d2a1f] transition-colors">
                  <td className="px-3 py-2">
                    {isEdit ? (
                      <input
                        value={item.name}
                        onChange={(e) => updateEditing(v.id, "name", e.target.value)}
                      ...
                      <span className="text-[#ECEDEE]">{v.name}</span>
                    )}
                  </td>
                  <td className="px-3 py-2">
                    {isEdit ? (
                      <input
                        value={item.slug}
                        onChange={(e) => updateEditing(v.id, "slug", e.target.value)}
                  ...
                  <td className="px-3 py-2">
                    {isEdit ? (
                      <input
                        value={item.emoji}
                        onChange={(e) => updateEditing(v.id, "emoji", e.target.value)}
                        className="w-16 px-2 py-1 bg-[#1a0f08] border border-[#4a3728] rounded text-[#ECEDEE] text-lg focus:outline-none focus:border-[#ee6c2b]"
                        maxLength={4}
                      />
                    ) : (
                      <span>{v.emoji ?? "—"}</span>
                  ...
                  <td className="px-3 py-2">
                    {isEdit ? (
                      <button
                        type="button"
                        onClick={() => updateEditing(v.id, "is_active", !item.is_active)}
                    ...
                    <div className="flex items-center gap-1">
                      {isEdit ? (
                        ...
                        <>
                          <button
                            onClick={() => startEdit(v)}
                            className="p-1 text-[#9BA1A6] hover:text-[#ee6c2b]"
                            title="Edit"
                          >
