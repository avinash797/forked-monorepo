      ...
      <View style={styles.content}>
        ...
        <View style={[styles.photoContainer, item.medal && styles.photoContainerOversized]}>
          {photoUrl ? (
            <Image
              source={{ uri: photoUrl }}
              style={[styles.photo, item.medal && styles.photoOversized]}
              contentFit="cover"
              transition={150}
            />
          ) : (
            <View style={[styles.photo, item.medal && styles.photoOversized, { backgroundColor: theme.color.surface2 }]} />
          )}
        </View>
        ...
        <View style={styles.infoContainer}>
          ...
          <View style={styles.metaRow}>
            ...
            <ThemedText style={styles.reviewCount}>
              {item.dish.review_count} {item.dish.review_count === 1 ? 'review' : 'reviews'}
            </ThemedText>
          </View>
        </View>
 ...
 */
function getMedalGradient(medal: 'gold' | 'silver' | 'bronze', theme: any): [string, string] {
  switch (medal) {
    case 'gold':
      return [theme.color.gold, adjustBrightness(theme.color.gold, 1.2)];
    case 'silver':
      return [theme.color.silver, adjustBrightness(theme.color.silver, 1.15)];
    case 'bronze':
      return [theme.color.bronze, adjustBrightness(theme.color.bronze, 1.15)];
    default:
      return ['#000', '#000'];
  }
}
