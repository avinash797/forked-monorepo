...
--
-- Apply after draft_01..04 (+ draft_seed). Idempotent.
--
--  * Configurable leaderboard qualification (app_constants) replaces the
--    hardcoded `total_ratings >= 2` in get_leaderboard, get_nearby_leaderboard,
--    get_dish_type_entry_counts, take_leaderboard_snapshot.
--  * evaluate_city_unlocks(): nightly, one-way city activation once >= 1 dish
--    type has enough qualifying entries. Locked/unlocked ONLY gates base-city
--    selection in the app header — rating works in any city regardless.
--  * match_location() now returns city.distance_meters so the client can fall
--    back to Nearby mode when the nearest active city is far away.
--  * get_flagship_board(): most active city+dish pair, for web marketing.
--  * Removes the dead Gemini-enrichment path (check_city_is_new,
--    trigger_enrich_city_dish_types) — dish types are seeded/admin-curated.
-- ============================================================
...
-- Grandfather already-active cities (New Orleans)
...
--    (bodies identical to draft_03 except the literal `2`)
-- ============================================================
...
-- 02:30 UTC, before the 03:00 snapshot so newly unlocked cities get a
-- day-one snapshot.
DO $$
BEGIN
...
--    cities.coordinates is a POINT, so the st_contains branch almost never
--    matches and the nearest-active fallback is the common path; the client
--    uses distance_meters to fall back to Nearby mode for far-away matches.
-- ============================================================
...
-- ============================================================
-- 5. get_flagship_board — most active (city, dish type) pair, for the
--    public marketing leaderboard preview. Anon-callable by design.
-- ============================================================
