...
-- rating system for dish rankings. This collapses the full 20260302..20260426
-- migration history into its FINAL state and folds in every later ALTER
-- (icons, placeholder photos, optional photos, location_properties, snapshot
-- scope, the account-deletion FK re-pointing, content-report review fields,
-- badges, and blocked_users).
--
-- Discrepancies fixed vs. the live database (see draft/README.md):
--   * content_reports ships WITH its review fields (reviewed_by / reviewed_at /
--     resolution_action / admin_notes) — these existed only in a migration file
--     that was never applied to prod, so the admin dashboard referenced columns
--     the live DB did not have.
--   * pg_trgm is created in the `extensions` schema, not `public`
--     (advisor: extension_in_public).
--
-- Foreign keys that PostgREST embeds depend on are named explicitly and MUST
-- keep their names: personal_ratings_user_id_fkey, comparisons_user_profile_fkey,
...
--
-- NOTE: intentionally has NO foreign key to auth.users. The account-deletion
-- flow (delete-account edge fn) anonymizes the profile into a tombstone and
-- then deletes the auth.users row; a CASCADE FK would wipe the tombstone and
-- every rating with it. Integrity for new users is maintained by the
-- handle_new_user() trigger (draft 4), not by an FK. (was 20260318000000)
-- ============================================================
...
-- ============================================================
-- PERSONAL RATINGS
-- (photo_url made nullable 20260311000000; user_id FK re-pointed to
--  profiles for the account-deletion tombstone design 20260318000000)
-- ============================================================
...
-- (user_id FK re-pointed to profiles 20260318000000)
-- ============================================================
...
-- (scope column + composite unique key added 20260309000000)
-- ============================================================
...
-- (photos backfilled/maintained 20260308180000)
-- ============================================================
...
-- (admin_id nullable + SET NULL on delete 20260318000000)
-- ============================================================
...
-- (reporter_id / reviewed_by re-pointed to profiles SET NULL 20260318000000)
-- ============================================================
...
--
-- FIX (migration drift): review fields shipped here. In prod these columns
-- lived only in an unapplied migration, so report-queries.ts referenced
-- columns the live DB lacked. reporter_id is nullable + SET NULL so a report
-- survives its reporter deleting their account (20260318000000).
-- ============================================================
...
-- ============================================================
-- FKs reference auth.users (NOT profiles) to match the deliberate final state
-- of migration 20260406000000: deleting a user cascade-removes their block rows
-- (both directions) rather than leaving them dangling against a tombstone.
