...
-- UTILITY: slugify
-- ============================================================
CREATE OR REPLACE FUNCTION public.slugify(v_text TEXT) RETURNS TEXT LANGUAGE plpgsql IMMUTABLE AS $$ BEGIN RETURN lower(
        regexp_replace(
...
IF v_user_id IS NULL THEN RAISE EXCEPTION 'User ID required';
END IF;
IF v_user_id != auth.uid() THEN RAISE EXCEPTION 'Can only anonymize your own data';
END IF;
...
-- ============================================================
CREATE OR REPLACE FUNCTION public._update_profile_stats(p_user_id UUID) RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public AS $$
...
SELECT COALESCE(SUM(pr.derived_score * p.credibility_score), 0),
...
-- ============================================================
CREATE OR REPLACE FUNCTION public.get_personal_dish_type_counts(p_user_id UUID DEFAULT NULL) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public AS $$
DECLARE v_user_id UUID;
v_result JSONB;
BEGIN v_user_id := COALESCE(p_user_id, auth.uid());
...
-- ============================================================
CREATE OR REPLACE FUNCTION public.check_city_is_new(p_city_id UUID) RETURNS TABLE (
        is_new BOOLEAN,
        city_name TEXT,
        city_state TEXT,
        city_country TEXT
...
AND p_lng IS NOT NULL THEN v_point := st_setsrid(st_point(p_lng, p_lat), 4326);
SELECT id INTO v_neighborhood_id
FROM public.neighborhoods
WHERE city_id = v_city_id
    AND st_contains(boundary::extensions.geometry, v_point)
LIMIT 1;
IF v_neighborhood_id IS NULL
AND p_neighborhood_name IS NOT NULL THEN
SELECT id INTO v_neighborhood_id
FROM public.neighborhoods
WHERE lower(name) = lower(p_neighborhood_name)
    AND city_id = v_city_id;
END IF;
ELSIF v_city_id IS NOT NULL
AND p_neighborhood_name IS NOT NULL THEN
SELECT id INTO v_neighborhood_id
FROM public.neighborhoods
WHERE lower(name) = lower(p_neighborhood_name)
...
SET name = EXCLUDED.name,
    ...
    neighborhood_id = COALESCE(
        public.restaurants.neighborhood_id,
        EXCLUDED.neighborhood_id
    ),
...
-- ============================================================
CREATE OR REPLACE FUNCTION public.search_restaurants(search_term TEXT) RETURNS SETOF public.restaurants LANGUAGE SQL STABLE AS $$
SELECT *
...
-- ============================================================
CREATE OR REPLACE FUNCTION public.search_dish_types(search_term TEXT) RETURNS SETOF public.dish_types LANGUAGE SQL STABLE AS $$
SELECT *
...
-- ============================================================
CREATE OR REPLACE FUNCTION public.get_admin_city_breakdown() RETURNS TABLE (
        city_id UUID,
        city_name TEXT,
...
SET search_path = public AS $$ WITH dates AS (
...
-- ============================================================
CREATE OR REPLACE FUNCTION public.get_admin_dish_type_breakdown() RETURNS TABLE (
        dish_type_id UUID,
        dish_type_name TEXT,
        total_ratings BIGINT,
        total_battles BIGINT,
...
SET search_path = public AS $$
SELECT dt.id AS dish_type_id,
    dt.name AS dish_type_name,
    (
        SELECT COUNT(*)
        FROM public.personal_ratings
        WHERE dish_type_id = dt.id
    ) AS total_ratings,
    (
        SELECT COUNT(*)
        FROM public.comparisons
    ...
    (
        SELECT ROUND(AVG(derived_score)::NUMERIC, 1)
        FROM public.personal_ratings
        WHERE dish_type_id = dt.id
    ) AS avg_score
