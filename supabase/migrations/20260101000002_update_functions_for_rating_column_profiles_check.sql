...
create or replace function public.get_helpful_reviews(
    ...
    user_id uuid,
    rating integer,
    -- Updated from star_rating to rating
    review_text text,
...
order by r.helpful_votes_count desc,
  r.created_at desc
limit p_limit;
end;
...
-- ============================================================================
create or replace function public.check_and_grant_charms(p_user_id uuid) returns integer as $$
declare v_charm record;
v_criteria jsonb;
v_granted_count integer := 0;
v_count integer;
begin -- Loop through all active charms
for v_charm in
select id,
  unlock_criteria
from public.charms
where is_active = true loop v_criteria := v_charm.unlock_criteria;
-- Skip if user already has this charm
if exists (
  select 1
  from public.user_charms
  where user_id = p_user_id
    and charm_id = v_charm.id
) then continue;
end if;
-- Check different criteria types
case
  v_criteria->>'type' -- Review count
  when 'review_count' then
  select count(*) into v_count
  from public.reviews
  where user_id = p_user_id
    and moderation_status = 'approved';
if v_count >= (v_criteria->>'min_count')::integer then perform public.grant_charm(p_user_id, v_charm.id);
v_granted_count := v_granted_count + 1;
end if;
-- Early adopter
when 'early_adopter' then if (
  select created_at
  from public.users
  where id = p_user_id
) < (v_criteria->>'join_before')::timestamp with time zone then perform public.grant_charm(p_user_id, v_charm.id);
v_granted_count := v_granted_count + 1;
end if;
...
where r.user_id = p_user_id
  and r.moderation_status = 'approved';
if v_count >= (v_criteria->>'min_cities')::integer then perform public.grant_charm(p_user_id, v_charm.id);
v_granted_count := v_granted_count + 1;
end if;
else -- Unknown criteria type, skip
continue;
end case
;
end loop;
